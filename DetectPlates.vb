...
Module DetectPlates
    ...
    Const PLATE_WIDTH_PADDING_FACTOR As Double = 1.3 '3.0
    Const PLATE_HEIGHT_PADDING_FACTOR As Double = 1.5 '3.0
    ...
    Function detectPlatesInScene(imgOriginalScene As Mat) As List(Of PossiblePlate)
        ...
        If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''''''
        ...
        If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''''''
            Koleso.txtInfo.AppendText("step 2 - listOfPossibleCharsInScene.Count = " + listOfPossibleCharsInScene.Count.ToString + vbCrLf)     '131 with MCLRNF1 image
        ...
        If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''''''
            Koleso.txtInfo.AppendText("step 3 - listOfListsOfMatchingCharsInScene.Count = " + listOfListsOfMatchingCharsInScene.Count.ToString + vbCrLf)     '13 with MCLRNF1 image
        ...
        Koleso.txtInfo.AppendText(vbCrLf + listOfPossiblePlates.Count.ToString + " possible plates found" + vbCrLf)    'update text box with # of plates found

        If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''''''
            Koleso.txtInfo.AppendText(vbCrLf)
            ...
            For i As Integer = 0 To listOfPossiblePlates.Count - 1
                ...
                CvInvoke.Imshow("4a", imgContours)
                Koleso.txtInfo.AppendText("possible plate " + i.ToString + ", click on any image and press a key to continue . . ." + vbCrLf)
                CvInvoke.Imshow("4b", listOfPossiblePlates(i).imgPlate)
                CvInvoke.WaitKey(0)
            Next
            Koleso.txtInfo.AppendText(vbCrLf + "plate detection complete, click on any image and press a key to begin char recognition . . ." + vbCrLf + vbCrLf)
            CvInvoke.WaitKey(0)
        End If ' show steps '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    Function findPossibleCharsInScene(imgThresh As Mat) As List(Of PossibleChar)
        ...
        If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''''''
            Koleso.txtInfo.AppendText(vbCrLf + "step 2 - contours.Size() = " + contours.Size().ToString + vbCrLf)      '2362 with MCLRNF1 image
            Koleso.txtInfo.AppendText("step 2 - intCountOfPossibleChars = " + intCountOfPossibleChars.ToString + vbCrLf)     '131 with MCLRNF1 image
