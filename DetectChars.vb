...
Module DetectChars
    ...
    Const MAX_CHANGE_IN_HEIGHT As Double = 1.2 '0.3
    ...
    Function loadKNNDataAndTrainKNN() As Boolean
        ...
        Catch ex As Exception                                                   'if error is encountered, show error and return
            Koleso.txtInfo.AppendText(vbCrLf + "unable to open 'classifications.xml', error: ")
            Koleso.txtInfo.AppendText(ex.Message + vbCrLf)
            Return False
        End Try
        ...
        Catch ex As Exception                                                   'if error is encountered, show error and return
            Koleso.txtInfo.AppendText(vbCrLf + "unable to open 'classifications.xml', error:" + vbCrLf)
            Koleso.txtInfo.AppendText(ex.Message + vbCrLf + vbCrLf)
            Return False
        End Try
        ...
        Catch ex As Exception                                               'if error is encountered, show error and return
            Koleso.txtInfo.AppendText("unable to open 'images.xml', error:" + vbCrLf)
            Koleso.txtInfo.AppendText(ex.Message + vbCrLf + vbCrLf)
            Return False
        End Try
    ...
    Function detectCharsInPlates(listOfPossiblePlates As List(Of PossiblePlate)) As List(Of PossiblePlate)
        ...
        For Each possiblePlate As PossiblePlate In listOfPossiblePlates     ' for each possible plate, this is a big for loop that takes up most of the function
            ...
            If (listOfListsOfMatchingCharsInPlate Is Nothing) Then          'if no matching chars were found
                If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''
                    Koleso.txtInfo.AppendText("chars found in plate number " + intPlateCounter.ToString + " = (none), click on any image and press a key to continue . . ." + vbCrLf)
            ...
            If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''
                Koleso.txtInfo.AppendText("chars found in plate number " + intPlateCounter.ToString + " = " + possiblePlate.strChars + ", click on any image and press a key to continue . . ." + vbCrLf)
                intPlateCounter = intPlateCounter + 1
                CvInvoke.WaitKey(0)
            End If ' show steps '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        If (Koleso.cbShowSteps.Checked = True) Then ' show steps '''''''''''''''''''''''''''''''''
            Koleso.txtInfo.AppendText(vbCrLf + "char detection complete, click on any image and press a key to continue . . ." + vbCrLf)
            CvInvoke.WaitKey(0)
        End If ' show steps '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
